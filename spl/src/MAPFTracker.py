...
def convert_solution_string(solution:str="", encoded:bool=True) -> str:
    ...
    for match in pattern.finditer(solution):
        ...
        if prev_position is not None:
            ...
            x = int(position[0])-int(position[0])
            y = int(position[1])-int(position[1])
    ...
    for move in motion:
        ...
        if move != prev_move:
            ...
            encoded_motions.append(f"{count}{prev_move}")
            count=1
        prev_move = move
    count = "" if count == 1 else count
    encoded_motions.append(f"{count}{prev_move}")
...
try:
    for fn in result: # ../result/Berlin_1_256-even-scen-1-agents-2.txt
        ...
        if (time.time() - time_last_debug) < DEBUG_PERIOD:
            ...
            print(f"ETA: {(((time_end-time_start)/debug_count)*(len(result)-debug_count))//60} minutes remaining. {debug_count}/{len(result)} solutions formatted.")
